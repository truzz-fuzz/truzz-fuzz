...
multi_seeds = glob.glob('./seeds/multi_*')
...
def process_data():
    ...
    multi_seeds_raw = glob.glob('./seeds/multi_*')
    multi_seeds = []
    for i in multi_seeds_raw:
        num = int(i[-6:])
        for j in range(num*3):
            multi_seeds.append(i)
...
def gen_mutate2(model, edge_num, sign):
    ...
    if round_cnt == 0:
        ...
        new_seed_list = new_seeds + multi_seeds

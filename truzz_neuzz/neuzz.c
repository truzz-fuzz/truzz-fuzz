...
char * init_trace_bits; 
typedef struct SListNode
{
  char* fn;
	float loc_check[10000];
  u8 init_value_loc[10000];
...
char* buf_for_check;
int file_len_global;
int wait_queue[10000][2]; 
int next_index = 0; 
int use_byte_analysis = 1;

int count_edge_nums(){
  int cur_edge_num = 0;
  for(int i = 0; i < MAP_SIZE;i++){
    if(trace_bits[i] != 0) cur_edge_num++;
  }
  return cur_edge_num;
}
...
float cal_score(){
  ...
  float cur_edge_num = 0;
  float init_edge_num = 0;
  float insertion_num = 0;

  for(int i = 0; i < MAP_SIZE;i++){
    if(trace_bits[i] != 0) cur_edge_num++;
    if(init_trace_bits[i] != 0) init_edge_num++;
    if(trace_bits[i] != 0 && init_trace_bits[i] != 0) insertion_num++;
  }

  if(cur_edge_num < init_edge_num)
    score = 0.5 * ( (init_edge_num - cur_edge_num)/init_edge_num + 1 - insertion_num/init_edge_num );
  else
    score = 0;
...
SLTNode* BuySListNode(char* fn){
	SLTNode* node = (SLTNode*)malloc(sizeof(SLTNode));
  node->fn = alloc_printf("%s", fn);
	node->next = NULL;
  memset(node->loc_check,0,sizeof(node->loc_check));
  memset(node->init_value_loc,0,sizeof(node->init_value_loc));
...
SLTNode* Search(char* fn){
  SLTNode* cur = head->next;
	while (cur)
	{
    if(strcmp(fn, cur->fn) == 0)
      return cur;
...
int out_buf_check1(char* buf){
  memset(loc_check,0,sizeof(loc_check));
  memset(buf_for_check,0,len);
  memcpy(buf_for_check, buf, len);

  u8 value = 0;

  write_to_testcase(buf_for_check, len);
  int fault = run_target(exec_tmout);

  u32 st_cksum = hash32(trace_bits, MAP_SIZE, HASH_CONST);
  memcpy(init_trace_bits, trace_bits, MAP_SIZE);
  wait_queue[0][0] = 0;
  wait_queue[0][1] = (file_len_global-1)/2;
  wait_queue[1][0] = (file_len_global-1)/2 + 1;
  wait_queue[1][1] = (file_len_global-1);
  next_index += 2;
  ...
  while(next_index != 0){
    ...
    next_index -= 1;
    int begin_test_index = wait_queue[next_index][0];
    int end_test_index = wait_queue[next_index][1];

    for(int t = begin_test_index;t <= end_test_index; t++){
      buf_for_check[t] = ~(u8)buf_for_check[t];
    }
    write_to_testcase(buf_for_check, len);
    int fault = run_target(exec_tmout);
    exec_num++;

    float score = cal_score();
    if(score > 0.2){
    
      if(end_test_index > begin_test_index + 2){
        wait_queue[next_index][0] = begin_test_index;
        wait_queue[next_index][1] = (begin_test_index + end_test_index)/2;
        wait_queue[next_index + 1][0] = (begin_test_index + end_test_index)/2 + 1;
        wait_queue[next_index + 1][1] = end_test_index;
        next_index += 2;
      }else{ 
        for(int t = begin_test_index;t <= end_test_index; t++){
          loc_check[t] = score;
      ...
      skip += (end_test_index - begin_test_index);
      skip_num++;
      for(int t = begin_test_index;t <= end_test_index; t++){
        loc_check[t] = score;
      }
  ...
  for(int i=0; i<file_len_global; i++ ){
    init_value_loc[i] = (u8)buf[i];
  }
  return 0;
}
...
void gen_mutate(){
    ...
    for(int iter=0 ;iter<13; iter=iter+1){
        ...
        for(int step=0;step<up_step;step=step+1){
            int mut_val;
            for(int index=low_index; index<up_index; index=index+1){
                if(use_byte_analysis && loc_check[loc[index]] > 0.2){
                  if((float)rand()/RAND_MAX < loc_check[loc[index]]){
                    out_buf1[loc[index]] = init_value_loc[loc[index]];
                  }else{
                    out_buf1[loc[index]] = rand() % 256;
                  }
                  continue;
                }
            ...
            if(ret == 2){
                int cur_num = count_non_255_bytes(virgin_bits);
                int discover_num = cur_num - pre_num;
                pre_num = cur_num;
                char* mut_fn;
                if(discover_num > 2)
                  mut_fn = alloc_printf("%s/multi_%d_%d_%06d_cov_%06d", out_dir, round_cnt, iter, mut_cnt, discover_num);
                else
                  mut_fn = alloc_printf("%s/id_%d_%d_%06d_cov", out_dir, round_cnt, iter, mut_cnt);
                int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
                ck_write(mut_fd, out_buf1, len, mut_fn);
                free(mut_fn);
                close(mut_fd);
    ...
    for(int del_count=0; del_count < 1024;del_count= del_count+1){
        ...
        if(ret==2){
            ...
            char* mut_fn;
            if(discover_num > 2)
              mut_fn = alloc_printf("%s/multi_%d_%06d_cov_%06d", out_dir,round_cnt, mut_cnt, discover_num);
            else
              mut_fn = alloc_printf("%s/id_%d_%06d_cov", out_dir,round_cnt, mut_cnt);
            int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
            ck_write(mut_fd, out_buf1, len-cut_len, mut_fn);
            free(mut_fn);
            close(mut_fd);
...
void write_new_model(){
  FILE *fp = NULL;
  fp = fopen("./time_and_edge.txt", "a");
  if (fp == NULL) {
      perror("fopen");
      exit(EXIT_FAILURE);
  }
...
void write_data(){
  ...
  fprintf(fp, "\n%s\n", nowtime);
  fprintf(fp, "edge num %d\n",count_non_255_bytes(virgin_bits));
  fclose(fp);
}

int update_flag = 0;
int model_count = 0;
float pre_byte_analysis[3];
int idx_byte_analysis = 0;
int count_byte_analysis = 0;
float pre_without_byte_analysis[3];
int idx_without_byte_analysis = 0;
int count_without_byte_analysis = 0;
time_t record_begin_time = 0;
...
void fuzz_lop(char * grad_file, int sock){
    if(model_count > 1){
      ...
      fclose(fp);
      if((pre_byte_analysis[0] + pre_byte_analysis[1] + pre_byte_analysis[2])/count_byte_analysis > (pre_without_byte_analysis[0] + pre_without_byte_analysis[1] + pre_without_byte_analysis[2])/count_without_byte_analysis)
        use_byte_analysis = 1;
      else if((pre_byte_analysis[0] + pre_byte_analysis[1] + pre_byte_analysis[2])/count_byte_analysis == (pre_without_byte_analysis[0] + pre_without_byte_analysis[1] + pre_without_byte_analysis[2])/count_without_byte_analysis){
        if(rand() % 10 > 4){
          use_byte_analysis = 1;
        }else{
          use_byte_analysis = 0;
    ...
    int line_cnt=0;
    int retrain_interval = 1;
    if(round_cnt == 0)
        retrain_interval = 15;
    time_t pre = time(NULL);
    int pre_edge_nums = count_non_255_bytes(virgin_bits);
    int model_begin_edges = count_non_255_bytes(virgin_bits);
    int target_line_cnt = 1000;
    while ((nread = getline(&line, &llen, stream)) != -1) {        
        line_cnt = line_cnt+1;

        int cur_edge_nums = count_non_255_bytes(virgin_bits);
        if(cur_edge_nums > pre_edge_nums){
          pre = time(NULL);
          pre_edge_nums = cur_edge_nums;
        }
        
        if(time(NULL) - pre > 30 || update_flag == 1){
          update_flag = 1;
          if (target_line_cnt == 1000 && !access("gradient_info_p",0) ){
            target_line_cnt = line_cnt + 10;
          } 
          if(line_cnt > target_line_cnt)
            break;
        ...
        need_analysis = 0;
        SLTNode* target_node = Search(fn);
        if(target_node == NULL){
          need_analysis = 1;
          target_node = BuySListNode(fn);
          loc_check = target_node->loc_check; 
          init_value_loc = target_node->init_value_loc; 
        ...
        if((line_cnt % 3) == 0){ 
            printf("line_cnt:%d   edge num %d\n",line_cnt,count_non_255_bytes(virgin_bits));
            fflush(stdout);
        }
        if(time(NULL) - record_begin_time > 3600){
          write_data();
          record_begin_time = time(NULL);
        }
        ...
        if(use_byte_analysis && need_analysis){
          out_buf_check1(out_buf);
        }
    ...
    free(line);
    fclose(stream);
    if(use_byte_analysis){
      pre_byte_analysis[idx_byte_analysis] = count_non_255_bytes(virgin_bits) - model_begin_edges;
      idx_byte_analysis = (idx_byte_analysis+1)%3;
      if(count_byte_analysis < 3)
        count_byte_analysis++;
    }else{
      pre_without_byte_analysis[idx_without_byte_analysis] = count_non_255_bytes(virgin_bits) - model_begin_edges;
      idx_without_byte_analysis = (idx_without_byte_analysis+1)%3;
      if(count_without_byte_analysis < 3)
        count_without_byte_analysis++;
    }
}
...
void start_fuzz(int f_len){
    ...
    if(!out_buf3)
        perror("malloc failed");
    buf_for_check = malloc(20000);
    if(!buf_for_check)
        perror("malloc failed");
    init_trace_bits = malloc(MAP_SIZE);
    if(!buf_for_check)
        perror("malloc failed");
    ...
    /* dry run seeds*/
    dry_run(out_dir, 2);
    pre_num = count_non_255_bytes(virgin_bits);
    write_data();
